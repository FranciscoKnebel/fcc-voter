...
module.exports = function ( app ) {
	...
	app.post( '/poll/vote/:ID', function ( req, res ) {
		console.log( "Searching for poll " + req.body.poll );
		...
		Poll.findOne( {
			...
			console.log( "Adding vote in poll " + req.body.poll );
			var vote = poll.voteFor( req.body.option );
			if ( vote !== null ) {
				console.log( "Vote added to poll " + req.body.poll );
				res.status( 200 ).send( "Vote added to poll " + req.body.poll );
			} else {
				console.log( "Option not found in poll " + req.body.poll );
				res.status( 404 ).send( "Option not found in poll " + req.body.poll );
			}
		} );
	} );
